...
if showPlots
    ...
    semilogx(freqDomain,watsonTemporalModel(freqDomain,simulatedPsiParams(1:3)).*simulatedPsiParams(4),'-k');
    ylim([-0.5 1.5]);
    xlabel('log stimulus Frequency [Hz]');
    ylabel('Relative response amplitude');
    title('Estimate of Watson TTF');
    hold on
    currentFuncHandle = plot(freqDomain,watsonTemporalModel(freqDomain,simulatedPsiParams(1:3)).*simulatedPsiParams(4),'-k');
...
for tt = 1:nTrials
    ...
    if mod(tt,baselineTrialRate) ~= 1
        ...
        thePacket.stimulus.values = thePacketOrig.stimulus.values(1:nonBaselineTrials+1,1:tt*stimulusStructPerTrial);
        thePacket.stimulus.timebase = thePacketOrig.stimulus.timebase(:,1:tt*stimulusStructPerTrial);

        
        
        % Simulate outcome with tfe
        [outcome, modelResponseStruct, thePacketOut, pctBOLD]  = tfeUpdate(tfeObj,thePacket,...,
                                                        'stimulusVec',stim,...,
                                                        'qpParams',myQpParams,...,
                                                        'rngSeed',rngSeed,...,
                                                        'boldLimitsSimulate',BOLDlimitsSim,...,
                                                        'boldMaxFit',BOLDmaxFit);
    ...
    if nonBaselineTrials > 0 && showPlots
        ...
        currentFuncHandle = plot(freqDomain,watsonTemporalModel(freqDomain,psiParamsQuest(1:3)).*psiParamsQuest(4),'-r');
